...
export default function QuestionDisplay({
  ...
  const answeredCount = Array.from(new Set(answers.map((a) => a.player_id))).length;
  ...
  return (
    <div className="w-full max-w-7xl mx-auto flex flex-col h-full items-center justify-between py-8">
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <svg className="w-full h-full absolute top-0 left-0 -rotate-90 transform" viewBox="0 0 36 36">
              ...
              />
              <path
                className={`drop-shadow-[0_0_5px_rgba(236,72,153,0.8)] ${timeLeft <= 5 ? "text-red-500" : "text-pink-500"}`}
                d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831"
                fill="none"
                stroke="currentColor"
            ...
            <span
              className={`font-headline-md text-2xl font-black z-10 ${
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-col flex-grow">
                <span className="text-xl md:text-2xl font-bold text-white shadow-black drop-shadow-md whitespace-normal break-words">
                  {option}
                </span>
      ...
      {status === GameStatus.REVEAL && Boolean(question.explanation || question.metadata?.explanation) && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          ...
          <p className="text-[#e3e0f9]/80 text-sm">{(question.explanation as string) || (question.metadata?.explanation as string)}</p>
        </motion.div>
      )}

      {/* Time Progress Bar */}
      <div className="fixed bottom-0 left-0 h-3 bg-surface-container-high w-full">
        <motion.div
          animate={{ width: `${progress}%` }}
          transition={{ ease: "linear", duration: 1 }}
          className={`h-full rounded-full shadow-[0_0_10px_rgba(236,72,153,0.5)] ${timeLeft <= 5 ? "bg-red-500" : "bg-gradient-to-r from-primary to-pink-500"}`}
        />
      </div>
    </div>
  );
