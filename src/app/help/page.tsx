    ...
    <main className="min-h-screen relative overflow-hidden pb-32">
      ...
      <header className="sticky top-0 z-50 bg-background/80 backdrop-blur-xl border-b border-white/10">
        <div className="flex justify-between items-center w-full px-6 py-4 max-w-4xl mx-auto">
          ...
          <h1 className="text-lg font-bold text-on-surface">Ajuda</h1>
      ...
      <div className="relative z-10 max-w-2xl mx-auto p-6 space-y-8">
        {/* Search */}
        <div className="glass-panel rounded-xl p-2 flex items-center gap-3 border border-white/10 focus-within:border-primary/50 transition-all">
          <Search className="w-5 h-5 text-on-surface/40 ml-2" />
          <input
            type="text"
            placeholder="Pesquisar ajuda..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="bg-transparent border-none focus:ring-0 w-full text-on-surface placeholder:text-on-surface/30"
          />
        </div>
          ...
          <div className="space-y-3">
            {filteredFAQ.map((item, idx) => (
              ...
              >
                ...
                >
                  <span className="font-bold text-on-surface group-hover:text-primary transition-colors pr-4">
                    {item.question}
                  </span>
                  <motion.div
                  ...
                  {openIndex === idx && (
                    ...
                    >
                      <div className="px-6 pb-5">
                        <p className="text-on-surface/60 leading-relaxed">{item.answer}</p>
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>
        ...
        <section className="mt-12 text-center space-y-6">
          <div className="glass-panel p-8 rounded-3xl border border-white/5 relative overflow-hidden">
            ...
            <h3 className="text-xl font-bold text-on-surface relative z-10 mb-2">Ainda precisas de ajuda?</h3>
            <p className="text-on-surface/50 mb-8 relative z-10">
              Os nossos navegadores de suporte estão disponíveis 24/7.
            </p>
            <button className="w-full py-4 bg-gradient-to-r from-tertiary to-primary-container text-on-primary font-bold rounded-xl active:scale-95 transition-transform shadow-[0_0_20px_rgba(208,188,255,0.4)] relative z-10 flex items-center justify-center gap-2">
          ...
          <div className="grid grid-cols-2 gap-4">
            {QUICK_LINKS.map((link, idx) => (
              <motion.div
                ...
                className="glass-panel p-4 rounded-xl border border-white/10 hover:border-primary/30 transition-all cursor-pointer text-center"
              >
