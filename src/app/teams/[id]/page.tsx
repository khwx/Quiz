  ...
  if (!team) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4">
        <p className="text-on-surface/50">Equipa não encontrada.</p>
        <Link href="/teams" className="text-primary font-bold">Voltar</Link>
      </div>
    );
  }
    ...
    <main className="min-h-screen relative overflow-hidden pb-24">
      ...
      <header className="sticky top-0 z-50 bg-background/80 backdrop-blur-xl border-b border-white/10">
        <div className="flex justify-between items-center w-full px-6 py-4 max-w-4xl mx-auto">
          ...
          <h1 className="text-lg font-bold text-on-surface">Detalhe da Equipa</h1>
          <div className="w-10" />
        </div>
      </header>
      ...
      <div className="relative z-10 max-w-4xl mx-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          >
            ...
            <div className="text-center md:text-left">
              ...
              <p className="text-sm text-on-surface/60">{memberCount} / {team.max_members} membros</p>
            </div>
          </motion.div>
          ...
          >
            <span className="text-xs text-on-surface/60 uppercase tracking-widest font-bold mb-3">Código de Convite</span>
            <div className="bg-surface-container px-4 py-3 rounded-xl border border-white/5 w-full flex items-center justify-between mb-4">
              <span className="text-lg font-bold text-primary tracking-widest" style={{ fontFamily: "Space Grotesk" }}>{team.pin}</span>
              <button onClick={handleCopy} className="text-on-surface/60 hover:text-white transition-colors">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs text-on-surface/50">{memberCount} / {team.max_members} Pilotos</span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {members.map((member) => (
              <div key={member.id} className="glass-panel rounded-xl p-4 flex items-center gap-3 relative overflow-hidden">
                {member.role === UserRole.HOST && <div className="absolute top-0 left-0 w-1 h-full bg-secondary" />}
                <div className="w-12 h-12 rounded-full bg-surface-container border-2 border-white/10 flex items-center justify-center">
                ...
                <div className="flex-1">
                  ...
                  <p className={`text-xs ${member.role === UserRole.HOST ? "text-secondary" : "text-on-surface/50"}`}>
            ...
            {members.length === 0 && (
              <p className="text-on-surface/30 text-sm col-span-2">Nenhum membro ainda.</p>
            )}
          </div>
        </motion.div>
