    ...
    const steps = [
        {
            ...
            color: "from-primary/20 to-primary-container/10",
        },
        {
            ...
            color: "from-secondary/20 to-secondary-container/10",
        },
        {
            ...
            color: "from-tertiary/20 to-tertiary-container/10",
        },
        {
            ...
            color: "from-green-500/20 to-[#2E7D32]/10",
        },
        {
            ...
            color: "from-amber-400/20 to-[#FF8C00]/10",
        }
    ];
        ...
        <main className="min-h-screen relative overflow-hidden pb-20">
            ...
            <div className="relative z-10 max-w-6xl mx-auto p-6">
                <div className="flex items-center justify-between mb-12">
                    <Link href="/" className="flex items-center gap-2 text-on-surface/60 hover:text-on-surface transition-colors group">
                        <ArrowLeft className="group-hover:-translate-x-1 transition-transform" />
                        Voltar ao Início
                    </Link>
                    <div className="flex items-center gap-2 px-4 py-1 bg-primary/10 rounded-full border border-primary/20">
                        <Sparkles size={16} className="text-primary" />
                        <span className="text-sm font-bold uppercase tracking-widest text-primary">Guia Completo</span>
                    </div>
                </div>
                ...
                >
                    <h1 className="text-4xl md:text-7xl font-bold mb-6 tracking-tight leading-none">
                        COMO <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-secondary">JOGAR</span> O QUIZ
                    </h1>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-16">
                    {features.map((feature, idx) => (
                        <motion.div
                            ...
                            className="bg-surface-container/80 backdrop-blur-xl border border-white/10 rounded-2xl p-6 text-center hover:border-white/20 transition-all"
                        >
                            <div className="mb-3 flex justify-center">{feature.icon}</div>
                            <h3 className="text-lg font-bold mb-2 text-on-surface">{feature.title}</h3>
                            <p className="text-on-surface/50 text-sm">{feature.description}</p>
                        </motion.div>
                    ))}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-20">
                    {steps.map((step, idx) => (
                        ...
                        >
                            <div className="w-16 h-16 bg-background/60 rounded-2xl flex items-center justify-center mb-6 border border-white/10 group-hover:border-white/20 transition-colors">
                                {step.icon}
                            </div>
                            <h3 className="text-2xl font-bold mb-3 text-on-surface">{step.title}</h3>
                ...
                <motion.div
                    ...
                    viewport={{ once: true }}
                    className="bg-surface-container/80 backdrop-blur-xl border border-white/10 rounded-[3rem] p-8 md:p-12 mb-20"
                >
                    <div className="flex items-center gap-3 mb-8">
                        <Trophy className="text-amber-400 w-8 h-8" />
                        <h2 className="text-3xl font-bold text-on-surface">DICAS DE CAMPEÃO</h2>
                    </div>
                ...
                >
                    <Link href="/host" className="inline-flex items-center gap-3 bg-primary text-on-primary font-bold px-12 py-5 rounded-2xl text-2xl hover:shadow-[0_0_30px_rgba(208,188,255,0.3)] transition-all transform hover:scale-105 active:scale-95">
                        <Rocket className="w-6 h-6" />
                        COMEÇAR AGORA
                    </Link>
                    <p className="mt-4 text-on-surface/40 font-medium">Não é necessário baixar nada. 100% Grátis!</p>
                </motion.div>
            </div>
        </main>
    );
